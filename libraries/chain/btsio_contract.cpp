...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::eosio_code_name ) // virtual btsio.code permission does not really exist but is allowed
         continue;
 ...
 */
void apply_btsio_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      EOS_ASSERT( name_str.find( "bts." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'bts.'" );
   }
...
void apply_btsio_setcode(apply_context& context) {
...
void apply_btsio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_btsio_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "bts." ) != 0, action_validate_exception,
               "Permission names that start with 'bts.' are reserved" );
...
void apply_btsio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_btsio_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_btsio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_btsio_canceldelay(apply_context& context) {
