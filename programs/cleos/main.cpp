...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "BTS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ("code", "btsio")("scope", "btsio")("table", "namebids")
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(bts.msig), N(propose), variant_to_bin( N(bts.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(bts.msig), name(action), variant_to_bin( N(bts.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(bts.msig), N(invalidate), variant_to_bin( N(bts.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(bts.msig), N(cancel), variant_to_bin( N(bts.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(bts.msig), N(exec), variant_to_bin( N(bts.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send bts.system contract action to the blockchain."));
